...
newsApp.config(['$routeProvider',
    function ($routeProvider) {
		...
		$routeProvider
			...
			.when('/underarmour/', {
				controller: 'sweepsSection',
				templateUrl: 'views/underarmour.html'
			})
			.when('/news/:section/', {
				controller: 'newsSection',
				templateUrl: 'views/section.html'
			...
			.when('/news/:section/:sub/:subsub', {
				controller: 'newsSection',
				templateUrl: 'views/section.html'
			})
			.when('/rankings/:tour', {
				controller: 'rankingsSection',
...
//						console.log($location);
		...
		// image
		.when('/photos/:year/:month/:day/:imageid/', {
		...
		.when('/photos/galleries/:year/:month/:day/:slug/:imageid', {
			controller: 'galleryImageCtrl',
			templateUrl: 'views/galleryimage.html'
		})
...
newsApp.factory('titler', function () {
	var title = '';
	var base = 'Golfweek Mobile';
	return {
		title: function () {
			if (title === '')
				return base;
...
newsApp.factory('gw', ['$http', '$log', '$q',
    function ($http, $log, $q) {
		...
		return {
			...
			ranking: function (rankingData, context) {
				$http.post('http://rankings.golfweek.com/mobile-json/' + rankingData.tour + '.asp')
					.success(function (data) {
						console.log(data);
						context.success(data);
					})
			...
			story: function (storyData, context) {
				...
				$http.post('http://golfweek.com/news/' +
					...
					.success(function (data) {
						...
						editme = editme.split('src="https://www.youtube.com/embed/').join('tube-player="');
			...
			section: function (section, context) {
				...
				category += (section.subsub === undefined) ? '' : '/' + section.subsub;
				category += (section.subsubsub === undefined) ? '' : '/' + section.subsubsub;
			...
			image: function (section, context) {

				var mediaurl = 'http://golfweek.com/json/image/';
				
				mediaurl += '?ID=' + section.imageid;
			...
			galleryimage: function (section, context) {

				var mediaurl = 'http://golfweek.com/json/galleryimage/';
			...
			search: function (section, context) {

				var mediaurl = 'http://golfweek.com/search/';
				
				mediaurl += '?q=' + section.query + '&amp;sortby=date';
...
newsApp.directive('htmlContent', ['$timeout', '$window',
    function ($timeout, $window) {
		...
		return {
			...
			link: function (scope, element, attrs) {
				$timeout(function () {
					element.html(scope.rankingData)
				}, 1500);
			}
		};
    }]);
...
newsApp.directive('storyTest', ['$log', '$http', '$location', '$timeout', '$interpolate', '$window',
    function ($log, $http, $location, $timeout, $interpolate, $window) {
		'use strict';
		...
		return {
			...
			link: function (scope, element, attrs) {
				...
				// to app pages  should bw

				var links = $('a');
				angular.forEach(links, function (val) {
//					console.log($(val).attr('href').indexOf('scores.golfweek') > -1, val);
					if ($(val).attr('href') && $(val).attr('href').indexOf('golfweek') > 1 && $(val).attr('href').indexOf('scores.golfweek') < 0) {
						$(val).attr('href', $(val).attr('href').replace('http://golfweek.com', '')); // website
						//	$(val).attr('href', $(val).attr('href').replace('http://golfweek.com', '#'));  // app
						// console.log(val);
					} else {
						$(val).click(function (e) {
							...
							// window.open(href, '_blank', 'location=no,toolbar=yes');  // app
						})
					}

				})
				
				//  replace all blockquotes with twitter functionality.
				//var blockquotes = $('blockquote')
				//angular.forEach(blockquotes, function (val) {
				//	var text = $(val).text();
				//	var newtext = text.replace(/\ /g, '%20');
				//	newtext = newtext.replace(/(?:\r\n|\r|\n)/g, '');
				//	var thisURL = 'http://www.golfweek.com' + $location.$$url; // story url
				//	var thisTwitter = "https://twitter.com/intent/tweet?text=" + newtext + "&url=" + thisURL
				//	var frame = "<a ng-click='" + thisTwitter + "' style='font-weight: normal'>" + text + "<img src='http://ads.golfweek.com/MegaMenu/img/twitter-2.png' class='blockquote-twitter' alt='Tweet this Quote on Twitter'></a>";
				//	$(val).html(frame);
				//})
...
newsApp.directive('galleryimage', ['$log',
    function ($log) {
		return {
			restrict: 'A',
			link: function (scope, element, attrs) {
				scope.$watch("galleryImageData", function () {
					element.html(scope.galleryImageData[attrs.galleryimage]);
				})
			}
		};
    }]);

newsApp.directive('image', ['$log',
    function ($log) {
		return {
			restrict: 'A',
			link: function (scope, element, attrs) {
				scope.$watch("imageData", function () {
					element.html(scope.imageData[attrs.image]);
				})
			}
		};
    }]);
		...
		function putAd(scope, elem) {
			...
			deferred.promise.then(function () {
				...
				if (document.getElementsByTagName('iframe').length === 1) {
					...
					googletag.cmd.push(function () {
						var adSlot = googletag.defineSlot('/310322/a.site152.tmus/mobile', [300, 50], elem);
...
newsApp.directive('interstitial', ['$q', '$window',
	function ($q, $window) {
		...
		function putAd(scope, elem) {
			...
			deferred.promise.then(function () {
				...
				if (document.getElementsByTagName('iframe').length === 1) {
					...
					googletag.cmd.push(function () {
						var adSlot = googletag.defineSlot('/310322/a.site152.tmus/mobile', [300, 251], elem);
						adSlot.addService(googletag.pubads());
						googletag.enableServices();
					});
